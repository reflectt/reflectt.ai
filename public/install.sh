...
echo ""
info "=== Next steps ==="
ok "Discover all endpoints:  curl http://127.0.0.1:${REFLECTT_NODE_PORT}/capabilities | jq ."
ok "Generate agent config:   curl http://127.0.0.1:${REFLECTT_NODE_PORT}/bootstrap/heartbeat/<agent_name>"
ok "Single heartbeat call:   curl http://127.0.0.1:${REFLECTT_NODE_PORT}/heartbeat/<agent_name>"
ok "Pull first task:         curl 'http://127.0.0.1:${REFLECTT_NODE_PORT}/tasks/next?agent=<agent_name>&compact=true'"
echo ""
ok "Tip: Add ?compact=true to most GET endpoints to reduce response size by 50-75%"
ok "Tip: Any 404 returns a markdown discovery page with available endpoints"
echo ""
ok "Cloud setup: https://app.reflectt.ai/bootstrap"
